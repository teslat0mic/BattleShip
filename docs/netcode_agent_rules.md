...
- Netplay input model: a controller sample taken at tick T is the input for tick `T + delay` on **both** machines (scheduled locally in `sSYNetInputLocalScheduled`, transmitted with that final tick). Never apply a local sample immediately while sending it delayed. A tick whose remote input has not arrived is **not simulated** (`syNetPeerCheckSimulationInputReady`); the peer waits rather than guessing, because nothing here can undo a wrong guess yet. Anything a peer derives from VI count rather than simulated ticks drifts while a peer waits — the battle clock subtracts waited VIs for exactly this reason.
- Testing netplay without a controller: `SSB64_NETPLAY_FAKE_INPUT=<seed>` replaces the local controller sample with a deterministic input-rich stream. **A netplay test with a still controller proves almost nothing** — prediction repeats the last input, so neutral inputs are predicted perfectly and hide desyncs (this masked three separate bugs). Always use fake input, or a human.
